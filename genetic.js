...
const mutationsChance = document.querySelector('#mutationValue');
const childsCount = document.querySelector('#childsValue');
...
const mutationRange = document.querySelector('#mutationSlider')
mutationRange.addEventListener('input', (e) => {
    mutationsS = e.target.value/100;
    mutationsChance.value = e.target.value/100;
}, true)
mutationsChance.addEventListener('input', (e) => {
    mutationsS = e.target.value/100
    mutationRange.value = e.target.value/100
}, true)

const childsRange = document.querySelector('#childsSlider')
childsRange.addEventListener('input', (e) => {
    childsS = e.target.value
    childsCount.value = e.target.value
}, true)
childsCount.addEventListener('input', (e) => {
    childsS = e.target.value
    childsRange.value = e.target.value
}, true)
...
function cross(father, mother)
{
  ...
  if (firstson.weight > secondson.weight)
  {
    return firstson;
  }
  else
...
function createNewPopulation(pop)
{
  newPop = pop;
  for (let i = 0; i < childsS.value; ++i)
  {
    let father = pop.allIndv[getRandomInt(pop.allIndv.length-1)];
    let mother = pop.allIndv[getRandomInt(pop.allIndv.length-1)];
    let child = cross(father, mother);
    //add new individuals into population
    pop.allIndv[populationSize.value - (i+1)] = child;
  }
  let mutationsCount = populationSize.value*mutationsChance.value;
  for (let i = childsS.value+1; i < mutationsCount+childsS.value+1; ++i)
  {
...
function geneticAlgorithm()
{
  ...
  {
    createNewPopulation(generation);
